...
void NeoPixelEffects::setFlashWhenEffectsChange(const bool value) {
  _flashWhenEffectsChange = value;
};
...
void NeoPixelEffects::chooseNewEffect() {
  if (effectsEnabled() > 0) {
    _currentEffectIndex++;
    _currentEffectIndex = _currentEffectIndex % _numEffectsEnabled;
    _currentEffect = _enabledEffects[_currentEffectIndex];
    i = 0;
    j = 0;

    if (_flashWhenEffectsChange) {
      _strip->clear();
      _strip->show();
      delay(100);
      _strip->clear();
...
void NeoPixelEffects::colorGray() {
  colorWipe(_strip->Color(60,60,60)); // Gray
}
void NeoPixelEffects::colorWhite() {
  colorWipe(_strip->Color(255,255, 255)); // White
}
...
void NeoPixelEffects::colorPurple() {
  colorWipe(_strip->Color(127, 20, 127)); // Red
}
...
void NeoPixelEffects::rainbowCycle() {
  for (i = 0; i < _strip->numPixels(); i++) {
    _strip->setPixelColor(i,
      Wheel(((i * 256 / _strip->numPixels()) + j) &
      255));
    }
    _strip->show();
    j++;
  ...
  void NeoPixelEffects::theaterChase(uint32_t c) {
    for (int p = 0; p < _strip->numPixels() * 2; p = p + 2) {
      _strip->setPixelColor((p) % _strip->numPixels(),
      c);    //turn every third pixel on
    }
